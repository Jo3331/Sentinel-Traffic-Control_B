...
def main():
    ...
    try:
        while True:
            ...
            # ── P0 Safety Fallback: force B-side RED on master signal loss ──
            # If Board B has not received a fresh LIGHT command from Board A
            # within LIGHT_CMD_TIMEOUT, never keep the previous green/yellow state.
            if not light_fresh:
                light_state = "NO_SIGNAL"
                light_remaining = 0.0
                light_color = (0, 0, 255)
                is_green = False
                is_yellow = False
                relay.set_light("RED")
            elif light_state == "GREEN_B":
